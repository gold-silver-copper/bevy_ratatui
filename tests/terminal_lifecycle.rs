...
//! cannot leak between scenarios. The parent captures a bounded terminal escape stream, applies a
//! hard timeout, optionally emulates kitty keyboard-protocol responses, and compares the PTY's
//! stable, restorable termios settings before and after the child.
...
use bevy_ratatui::{RatatuiContext, RatatuiPlugins, context::ContextSetup, kitty::KittyEnabled};
use nix::{
    errno::Errno,
...
const NESTED_CONTEXT_REJECTED: &[u8] = b"nested-context-rejected=true";
...
impl ProbeMode {
    ...
    fn parse(value: &str) -> Option<Self> {
        Some(match value {
            "panic-update" => Self::PanicUpdate,
            "panic-pre-startup" => Self::PanicPreStartup,
            "panic-startup" => Self::PanicStartup,
            "panic-post-update" => Self::PanicPostUpdate,
            "panic-runner" => Self::PanicRunner,
            "custom-hook-panic" => Self::CustomHookPanic,
            "previous-hook-panics" => Self::PreviousHookPanics,
            "exit" => Self::Exit,
            "drop" => Self::Drop,
            "direct-drop" => Self::DirectDrop,
            "sequential" => Self::Sequential,
            "sequential-hooks" => Self::SequentialHooks,
            "already-raw" => Self::AlreadyRaw,
            "nested" => Self::Nested,
            "non-tty" => Self::NonTty,
            #[cfg(feature = "mouse")]
            "mouse" => Self::Mouse,
            "kitty-supported" => Self::KittySupported,
            "kitty-unsupported" => Self::KittyUnsupported,
            _ => return None,
        })
    }
...
/// Cleanup must finish before the prior panic hook writes diagnostics, and ownership must prevent
/// the later app/resource drops from emitting a second terminal restoration.
#[test]
...
/// boundary, not just from an `Update` system.
#[test]
...
/// Custom and run-once runners may return without `AppExit`; app ownership must still clean up.
#[test]
...
/// Finished apps must reinstate the hook they replaced. A final panic must not replay any cleanup
/// hook from a prior session.
#[test]
fn many_sequential_apps_do_not_leave_stale_cleanup_hooks() {
    if run_child_if_requested(ProbeMode::Sequential) {
        return;
    }

    let probe = run_probe(
        "many_sequential_apps_do_not_leave_stale_cleanup_hooks",
...
/// A later session must capture the hook that exists when that session begins, rather than the
/// hook that existed when an earlier session began.
#[test]
...
/// A direct context owns the token until drop, without a plugin session that could restore twice.
#[test]
...
/// Initialization must reject a terminal already in raw mode without leaving the alternate screen
...
fn preexisting_raw_mode_is_rejected_and_preserved() {
    ...
    assert_probe_succeeded(&probe);
    assert_terminal_attributes_restored(&probe);
    assert!(contains(&probe.output, ALREADY_RAW_PRESERVED));
    assert_eq!(count(&probe.output, ENTER_ALTERNATE_SCREEN), 0);
    assert_eq!(count(&probe.output, LEAVE_ALTERNATE_SCREEN), 0);
}

/// A second context attempt while the first owns raw mode must fail without disturbing the first.
#[test]
fn nested_context_is_rejected_and_first_owner_recovers_once() {
    if run_child_if_requested(ProbeMode::Nested) {
        return;
    }

    let probe = run_probe(
        "nested_context_is_rejected_and_first_owner_recovers_once",
        ProbeMode::Nested,
        ProbeOptions::default(),
    );
    assert_probe_succeeded(&probe);
    assert_terminal_attributes_restored(&probe);
    assert!(contains(&probe.output, NESTED_CONTEXT_REJECTED));
    assert_eq!(count(&probe.output, ENTER_ALTERNATE_SCREEN), 1);
    assert_eq!(count(&probe.output, LEAVE_ALTERNATE_SCREEN), 1);
}

/// With no controlling terminal, initialization must fail without emitting terminal setup or
/// cleanup escapes. This subprocess intentionally runs without a PTY.
#[test]
...
/// Mouse capture must be released before the terminal buffer is restored.
#[cfg(feature = "mouse")]
#[test]
...
/// An emulated supporting terminal must cause one kitty push and one pop before terminal cleanup.
#[test]
...
fn run_child(mode: ProbeMode) {
    match mode {
        ...
        ProbeMode::AlreadyRaw => {
            ...
            let initialization = RatatuiContext::init();
            assert!(initialization.is_err(), "raw terminal was accepted");
            ...
            println!("already-raw-preserved={preserved}");
        }
        ProbeMode::Nested => {
            let first = RatatuiContext::init().expect("initialize first context");
            let rejected = RatatuiContext::init().is_err();
            assert!(
                is_raw_mode_enabled().expect("first context still owns raw mode"),
                "nested initialization disturbed the first context"
            );
            drop(first);
            println!("nested-context-rejected={rejected}");
        }
        ProbeMode::NonTty => {
            setsid().expect("detach non-TTY probe from its controlling terminal");
            let rejected = RatatuiContext::init().is_err();
        ...
        ProbeMode::KittyUnsupported => run_app(
            AppAction::None,
            AppSettings {
                kitty: true,
...
fn open_lifecycle_pty() -> OpenptyResult {
    ...
    for attempt in 1..=MAX_ATTEMPTS {
        match openpty(Some(&winsize), None) {
            ...
            {
                // Parallel lifecycle cases can briefly contend for PTYs or descriptor slots,
                // especially in the stress loop. A bounded retry keeps that infrastructure noise
                // from hiding lifecycle failures while persistent allocation errors still fail.
...
#[test]
fn cleanup_escape_order_is_terminal_then_cursor_show() {
    if run_child_if_requested(ProbeMode::Drop) {
        return;
    }

    let probe = run_probe(
        "cleanup_escape_order_is_terminal_then_cursor_show",
        ProbeMode::Drop,
        ProbeOptions::default(),
    );
    assert_probe_succeeded(&probe);
